...
{
    [Activity(ScreenOrientation = a.Content.PM.ScreenOrientation.Portrait, Label = "", Icon = "@drawable/icon", Theme = "@style/CustomActionBarTheme",WindowSoftInputMode = SoftInput.AdjustPan)]
    public class ListEditAdd : Activity
    {
        ...
        private void BtnAddColName_Click(object sender, EventArgs e)//Add column to list
        {
            ...
            alert.SetView(input);
            //User confirmation
            alert.SetNegativeButton("Summary", (senderAlert, args) => {
                AddColumn(input.Text, Column.ColumnType.Title);
            });
            alert.SetNeutralButton("Other", (senderAlert, args) => {
